...
selectdata<-subset(data,select=selectdataname)
...
alldata<-cbind(selectdata,subject,activity)
...
selectdata_dedup<-subset(alldata, select=which(!duplicated(names(alldata))))
