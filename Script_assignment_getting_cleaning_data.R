...
# A subject file containing whose data records are recorded
...
alldata_dedup<-subset(alldata, select=which(!duplicated(names(alldata))))

average_data<-  alldata_dedup %>%
...
#library(memisc)
#library(codebook)
#CodeBook<-codebook(average_data)
#write_html(CodeBook,file="CodeBook.md")
